  ...
  _createClass(Service, [{
    ...
    value: function createQuery(originalQuery) {
      ...
      if (query.id) {
        ...
        var $in = query.id.$in;
        var index = query.id.$index;
        rq = $in ? (_rq = rq).getAll.apply(_rq, _toConsumableArray($in).concat(_toConsumableArray(index ? [{ index: index }] : []))) : rq.get(query.id);
        delete query.id;
      }
      ...
      if (filters.$sort) {
        ...
        var _sorts = Object.entries(filters.$sort).map(function (_ref3) {
        ...
        rq = (_rq3 = rq).orderBy.apply(_rq3, _toConsumableArray(_sorts));
      }
    ...
    value: function _find() {
      ...
      // Execute the query
      return Promise.all([q, countQuery]).then(function (_ref5) {
